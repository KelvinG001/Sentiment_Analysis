...
pd = pd.read_excel('data/住みたい街ランキング2024 (version 1).xlsx')
...
async def manually_get_tweets(tweets, query):
    if tweets is None: #begin search at every new round
        ...
        print(f'{datetime.now()} - Collecting more tweets after {wait} seconds...')
        time.sleep(wait)
    ...
    for query in queries:
        ...
        while tweet_count < min_tweet:
            ...
            for tweet in tweets:
                ...
                with open(f'tweets_info_{q}.csv', 'a', newline='', encoding='utf-8-sig', errors='replace') as file: #save the data into a csv file
                    csv_file = csv.writer(file)
